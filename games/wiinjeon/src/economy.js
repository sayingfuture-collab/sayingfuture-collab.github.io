 ...
 * ── 4배로 올렸다 (2026-08-22) ──
 *
 * 재보니 **네 명을 20렙으로 만드는 데 824판 · 27.5시간**이었다(20렙 하나가 24,700골드인데
 * 60층 판 하나가 120골드였다). 이건 어려운 게 아니라 **할 게 없는 것**이다 —
 * 골드가 막히면 강화도 뽑기도 막혀서, 남는 일이 같은 층을 다시 오르는 것뿐이 된다.
 *
 * ⚠️ **반복 수입도 같이 4배가 된다.** 「같은 층을 계속 도는 게 최적」이 되지 않는지는
 * 늘 다시 봐야 한다. 지금은 116층 한 판(×8배속 2분)에 928골드라
 * 네 명 20렙까지 106판 · 약 3.5시간이다.
 */
export const FLOOR_GOLD = 8;
/** 최고 기록을 1층 깰 때마다. 위와 같은 이유로 4배 */
export const RECORD_GOLD = 800;
